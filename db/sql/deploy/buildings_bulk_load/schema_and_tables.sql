-- Deploy buildings:buildings_bulk_load/schema_and_tables to pg
...
-- buildings_bulk_load
CREATE SCHEMA buildings_bulk_load;
...
CREATE TABLE buildings_bulk_load.organisation (
...
CREATE TABLE buildings_bulk_load.bulk_load_status (
...
CREATE TABLE buildings_bulk_load.qa_status (
...
CREATE TABLE buildings_bulk_load.supplied_datasets (
...
-- from the supplier.
CREATE TABLE buildings_bulk_load.supplied_outlines (
...
CREATE TABLE buildings_bulk_load.bulk_load_outlines (
...
-- supplied_dataset_id.
CREATE TABLE buildings_bulk_load.existing_subset_extracts (
...
CREATE TABLE buildings_bulk_load.added (
...
CREATE TABLE buildings_bulk_load.removed (
...
CREATE TABLE buildings_bulk_load.related_groups (
...
CREATE TABLE buildings_bulk_load.related (
...
-- loaded into the system in bulk and those that already exist.
CREATE TABLE buildings_bulk_load.matched (
      bulk_load_outline_id integer PRIMARY KEY REFERENCES buildings_bulk_load.bulk_load_outlines (bulk_load_outline_id)
    , building_outline_id integer NOT NULL REFERENCES buildings_bulk_load.existing_subset_extracts (building_outline_id)
    , qa_status_id integer NOT NULL REFERENCES buildings_bulk_load.qa_status (qa_status_id)
);
...
CREATE TABLE buildings_bulk_load.transferred (
      bulk_load_outline_id integer PRIMARY KEY REFERENCES buildings_bulk_load.bulk_load_outlines (bulk_load_outline_id)
...
CREATE TABLE buildings_bulk_load.deletion_description (
      bulk_load_outline_id integer PRIMARY KEY REFERENCES buildings_bulk_load.bulk_load_outlines (bulk_load_outline_id)
    , description character varying(250) NOT NULL
);
