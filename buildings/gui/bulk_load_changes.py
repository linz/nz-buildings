...
class BulkLoadChanges:
    ...
    def populate_edit_comboboxes(self):
        ...
        if self.bulk_load_frame.rad_edit.isChecked():
            ...
            result = self.bulk_load_frame.db._execute(sql, (self.bulk_load_frame.bulk_load_outline_id,))
            result = result.fetchall()[0][0]
            self.bulk_load_frame.cmb_status.setCurrentIndex(self.bulk_load_frame.cmb_status.findText(result))
            ...
            result = self.bulk_load_frame.db._execute(sql, (self.bulk_load_frame.bulk_load_outline_id,))
            result = result.fetchall()[0][0]
            self.bulk_load_frame.cmb_capture_method_2.setCurrentIndex(self.bulk_load_frame.cmb_capture_method_2.findText(result))
            ...
            for index, item in enumerate(ls):
                if item == result:
                    value_index = index
            self.bulk_load_frame.cmb_capture_source.setCurrentIndex(value_index)
            ...
            result = self.bulk_load_frame.db._execute(sql, (self.bulk_load_frame.bulk_load_outline_id,))
            result = result.fetchall()[0][0]
            self.bulk_load_frame.cmb_suburb.setCurrentIndex(self.bulk_load_frame.cmb_suburb.findText(result))
            ...
            result = self.bulk_load_frame.db._execute(sql, (self.bulk_load_frame.bulk_load_outline_id,))
            result = result.fetchall()[0][0]
            self.bulk_load_frame.cmb_town.setCurrentIndex(self.bulk_load_frame.cmb_town.findText(result))
            ...
            result = self.bulk_load_frame.db._execute(sql, (self.bulk_load_frame.bulk_load_outline_id,))
            result = result.fetchall()[0][0]
            self.bulk_load_frame.cmb_ta.setCurrentIndex(self.bulk_load_frame.cmb_ta.findText(result))
...
class AddBulkLoad(BulkLoadChanges):
    ...
    def edit_ok_clicked(self, commit_status):
        ...
        result = self.bulk_load_frame.db.execute_no_commit(sql, (text_ls[0],
                                                                 text_ls[1]
                                                                 ))
        ...
        self.bulk_load_frame.db.execute_no_commit(sql, (self.bulk_load_frame.outline_id, ))
    ...
    def creator_feature_added(self, qgsfId):
        ...
        result = self.bulk_load_frame.db._execute(sql, data=(wkt, ))
...
class EditBulkLoad(BulkLoadChanges):
    ...
    def edit_ok_clicked(self, commit_status):
        ...
        if self.bulk_load_frame.geom_changed and not self.bulk_load_frame.select_changed:
            for key in self.bulk_load_frame.geoms:
                ...
                self.bulk_load_frame.db.execute_no_commit(sql,
                                                          (self.bulk_load_frame.geoms[key],
                                                           key))
        # if only attributes are changed
        if self.bulk_load_frame.select_changed and not self.bulk_load_frame.geom_changed:
            # bulk load status
            ...
            if len(self.bulk_load_frame.ids) > 0:
                ...
                sql = 'SELECT buildings_bulk_load.bulk_load_outlines_update_attributes(%s, %s, %s, %s, %s, %s, %s);'
                self.bulk_load_frame.db.execute_no_commit(sql, (self.bulk_load_frame.bulk_load_outline_id,
                                                                bulk_load_status_id,
                                                                capture_method_id,
                                                                capture_source_id,
                                                                suburb,
        ...
        if self.bulk_load_frame.geom_changed and self.bulk_load_frame.select_changed:
            for key in self.bulk_load_frame.geoms:
                sql = 'SELECT buildings_bulk_load.bulk_load_outlines_update_shape(%s, %s);'
                self.bulk_load_frame.db.execute_no_commit(sql, (self.bulk_load_frame.geoms[key],
                                                                key))
            # bulk load status
            text = self.bulk_load_frame.cmb_status.currentText()
            sql = 'SELECT bulk_load_status_id FROM buildings_bulk_load.bulk_load_status bls WHERE bls.value = %s;'
            ...
            if len(self.bulk_load_frame.ids) > 0:
                ...
                for i in self.bulk_load_frame.ids:
                    sql = 'SELECT buildings_bulk_load.bulk_load_outlines_update_attributes(%s, %s, %s, %s, %s, %s, %s);'
                    self.bulk_load_frame.db.execute_no_commit(sql, (i, bulk_load_status_id,
                                                                    capture_method_id,
                                                                    capture_source_id,
                                                                    suburb,
                                                                    town, t_a))
