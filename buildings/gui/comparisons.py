...
def compare_outlines(self, commit_status):
    ...
    if len(results) == 0:
        ...
        # This assumes you have a facilities_lds schema in your db.
        sql = "UPDATE buildings_bulk_load.bulk_load_outlines blo set bulk_load_use_id = use_id FROM (SELECT bulk_load_outline_id, CASE use WHEN 'hospital' THEN 16 WHEN 'school' THEN 27 END use_id FROM (SELECT bulk_load_outline_id, use FROM buildings_bulk_load.bulk_load_outlines blo JOIN ((WITH added_facility_intersects AS ( SELECT bulk_load_outline_id, lower(fac.use) AS use, ST_Area(ST_Intersection(supplied.shape, fac.shape)) / ST_Area(supplied.shape) * 100 AS 'supplied_intersect' FROM facilities_lds.nz_facilities fac JOIN (SELECT bulk_load_outline_id, qa_status_id, shape FROM buildings_bulk_load.added supplied JOIN buildings_bulk_load.added_outlines addedshapes USING (bulk_load_outline_id) WHERE qa_status_id = 1) supplied ON ST_Intersects(supplied.shape, fac.shape)) SELECT * FROM added_facility_intersects WHERE 'supplied_intersect' > 50)) s1 USING (bulk_load_outline_id)) use_select) added WHERE added.bulk_load_outline_id = blo.bulk_load_outline_id;"
        self.db.execute_no_commit(sql)
