...
class BulkLoadOutlines(QFrame, FORM_CLASS):
    ...
    def find_suburb(self):
        ...
        sql = sql + "(ST_Area(ST_Intersection(outlines.shape,ST_Transform(suburbs.shape, 2193)))/ST_Area(outlines.shape)) AS proportion "
        sql = sql + "FROM buildings_bulk_load.bulk_load_outlines AS outlines, admin_bdys.nz_locality AS suburbs "
        sql = sql + "WHERE ST_Intersects(outlines.shape,ST_Transform(suburbs.shape, 2193)) "
    ...
    def find_territorial_auth(self):
        ...
        sql = sql + "(ST_Area(ST_Intersection(outlines.shape,ST_Transform(tas.shape, 2193)))/ST_Area(outlines.shape)) AS proportion "
        sql = sql + "FROM buildings_bulk_load.bulk_load_outlines AS outlines, admin_bdys.territorial_authority AS tas "
        sql = sql + "WHERE ST_Intersects(outlines.shape,ST_Transform(tas.shape, 2193)) "
