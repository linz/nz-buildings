...
class ProcessPublish(unittest.TestCase):
    ...
    def test_duplicate_ids_check_on_publish(self):
        ...
        self.assertEqual(
            self.bulk_load_frame.check_dialog.tbl_dup_ids.model().rowCount(), 1
        )
        self.assertEqual(
            self.bulk_load_frame.check_dialog.tbl_dup_ids.model().item(0, 0).text(),
            "2003",
        )
        self.assertEqual(
            self.bulk_load_frame.check_dialog.tbl_dup_ids.model().item(0, 1).text(),
            "Added",
        )
        self.assertEqual(
            self.bulk_load_frame.check_dialog.tbl_dup_ids.model().item(0, 2).text(),
            "Matched",
        )
