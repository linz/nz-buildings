...
class ProcessNewEntryGuiTest(unittest.TestCase):
    ...
    def test_new_lifecycle_stage(self):
        ...
        result2 = result2.fetchall()[0][0]
        self.assertEqual(result2, result + 1)
        sql = "DELETE FROM buildings.lifecycle_stage WHERE lifecycle_stage_id = (SELECT MAX(lifecycle_stage_id) FROM buildings.lifecycle_stage)"
        db.execute(sql)
    ...
    def test_duplicate_lifecycle_stage(self):
        ...
        self.assertEqual(result2, result)
        if result != result2:
            sql = "DELETE FROM buildings.lifecycle_stage WHERE lifecyle_stage_id = (SELECT MAX(lifecycle_stage_id) FROM buildings.lifecycle_stage)"
            db.execute(sql)
