...
def update_canals(kx_api_key):

    # get last update of layer date from log
    from_var = db._execute(reference_select.update_log_canal_date)
    ...
    uri = 'srsname=\'EPSG:2193\' typename=\'data.linz.govt.nz:layer-50251-changeset\' url=\'https://data.linz.govt.nz/services;key={0}/wfs/layer-50251-changeset?viewparams=from:{1};to:{2}\' version=\'auto\' table="" sql='.format(
        kx_api_key, from_var, to_var)
    layer = QgsVectorLayer(uri, "canal_changeset", "WFS")
    ...
    for feature in layer.getFeatures():
        if feature.attribute('__change__') == 'DELETE':
            sql = 'SELECT buildings_reference.canal_polygons_delete_by_external_id(%s)'
            db.execute(sql, (feature.attribute('t50_fid'),))

        elif feature.attribute('__change__') == 'INSERT':
            result = db._execute(reference_select.canal_polygon_id_by_external_id, (feature.attribute('t50_fid'),))
            result = result.fetchone()
            if result is None:
                sql = 'SELECT buildings_reference.canal_polygons_insert(%s, %s)'
