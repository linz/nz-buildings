        ...
        for key, attributes in attr_dict.iteritems():
            if key == 1:
                for attr in attributes:
                    create_line_layer(
                        registry, symbol, attr[0], attr[1]  # color  # width
                    )
            else:
                for attr in attributes:
                    create_marker_layer(
...
def create_marker_layer(
    registry, symbol, marker, color, outline_color, size, ptype, owidth
):
...
class LayerRegistry(object):
    ...
    def add_postgres_layer(self, name, db_name, geomcolumn, schema, key, sql):
        self.group = (
            QgsProject.instance().layerTreeRoot().findGroup("Building Tool Layers")
        )
        try:
